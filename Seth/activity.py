from Seth.local_libs.activity_search import ActivitySearch
from Seth.commander import Commander
...
class Activity(Commander):
    ...
    def calculate(self, text):
        default_text_array = ['Seth', 'wake', 'up']
        ...
        if command_text_array == default_text_array:
            chances2 = self.get_chances(['Seth', 'wakeup'], user_text_array)
            if chances2 > chances:
                chances = chances2
        if chances > 70.0:
            return True
